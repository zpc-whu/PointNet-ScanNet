...
def eval_whole_scene_one_epoch(sess, ops, test_writer):
    ...
    tempStr = '%d\t' % (EPOCH_CNT)
    tempStr += '%f\t' % (loss_sum / float(num_batches))
    tempStr += '%f\t' % (total_correct_vox / float(total_seen_vox))
    tempStr += '%f\t' % (np.mean(np.array(total_correct_class_vox[1:])/(np.array(total_seen_class_vox[1:],dtype=np.float)+1e-6)))
    tempStr += '%f\t' % (total_correct / float(total_seen))
    tempStr += '%f\t' % (np.mean(np.array(total_correct_class[1:])/(np.array(total_seen_class[1:],dtype=np.float)+1e-6)))
    tempStr += '%f\t' % (caliacc)
    LOG_FOUT_RES.write(tempStr+'\n')
    LOG_FOUT_RES.flush()
